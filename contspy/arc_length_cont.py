...
def pseudo_arclength_cont(problem, x0, lmbda0, step_size0, max_steps, callback, newton_abs_tol=1.0e-10, newton_rel_tol=1.0e-10, newton_max_iters=200):
  """
  ...
  x, _ = solve_Newton(lambda x: problem.Res(x, lmbda), lambda x: problem.Jac(x, lmbda), x0)
  ...
  while True:
    ...
    x, lmbda, newton_success = Newton_corrector(problem, x, lmbda, x_current, lmbda_current, dx_ds, dlmbda_ds, ds, abs_tol=newton_abs_tol, rel_tol=newton_rel_tol)
    ...
    if stability != stability_current:
      if ~oscillation:
        ...
        x, lmbda, newton_success = turning_point_locator(problem, x_current, lmbda_current)
        lmbda_turning.append(lmbda)
      else:
        print(colored('Hopf point tracking algorithm!', 'red'))
        x, lmbda, newton_success = hopf_point_locator(problem, x_current, lmbda_current, eigvals, eigvecs)
  ...
  print('Detected {} turning points and {} Hopf bifurcation points.'.format(len(lmbda_turning), len(lmbda_hopf)))
  return np.array(lmbda_turning), np.array(lmbda_hopf)

def Newton_corrector(problem, x, lmbda, x_current, lmbda_current, dx_ds, dlmbda_ds, ds, abs_tol=1.0e-10, rel_tol=1.0e-10, max_iters=1000):
  ...
  while num_newton_steps < max_iters:
    ...
    if Rnrm < abs_tol or np.divide(Rnrm, Rnrm0) < rel_tol:
      if Rnrm < abs_tol:
        ...
        reason = 'REL_TOL'
      print(f"Solve converged due to {reason} after {num_newton_steps} iterations.")
...
def turning_point_locator(problem, x_current, lmbda_current, abs_tol=1.0e-04, rel_tol=1.0e-04, max_iters=1000):
  # Initialization
  x = x_current.copy()
  ...
  while num_newton_steps < max_iters:
    ...
    Resd = np.dot(problem.Jac(x + eps3 * b, lmbda), y) / eps3 + np.dot(problem.Jac(x, lmbda + eps1), y) / eps1 - np.dot(J, y) * (1.0 / eps3 + 1.0 / eps1)
...
def hopf_point_locator(problem, x_current, lmbda_current, eigenvals_current, eigenvecs_current, abs_tol=1.0e-04, rel_tol=1.0e-04, max_iters=1000):
  # Initialization
  x = x_current.copy()
  lmbda = lmbda_current.copy()
  ...
  Ry = np.dot(problem.Jac(x, lmbda), y) + omega * z # here I assume that B is the identity matrix (neglect thermal pressurization)
  Rz = np.dot(problem.Jac(x, lmbda), z) - omega * y # here I assume that B is the identity matrix (neglect thermal pressurization)
  Romega = np.dot(phi, y) - 1.0
  Rlmbda = np.dot(phi, z)

  Rnrm = np.sqrt(np.linalg.norm(Rx) ** 2 + np.linalg.norm(Ry) ** 2 + np.linalg.norm(Rz) ** 2 + Romega ** 2 + Rlmbda ** 2)
  Rnrm0 = Rnrm
  ...
  while num_newton_steps < max_iters:
    ...
    J_hopf = np.block([[J, omega * np.identity(Rx.size)], [-omega * np.identity(Rx.size), J]])
    ...
    eps2 = delta * (np.linalg.norm(x) / np.linalg.norm(a) + delta)
    Rese = (np.dot(problem.Jac(x + eps2 * a, lmbda), y) - np.dot(problem.Jac(x, lmbda), y)) / eps2
    Resf = (np.dot(problem.Jac(x + eps2 * a, lmbda), z) - np.dot(problem.Jac(x, lmbda), z)) / eps2
    ...
    eps3 = delta * (np.linalg.norm(x) / np.linalg.norm(b) + delta)
    Resg = np.dot(problem.Jac(x + eps3 * b, lmbda), y) / eps3 + np.dot(problem.Jac(x, lmbda + eps1), y) / eps1 - np.dot(J, y) * (1.0 / eps3 + 1.0 / eps1)
    Resh = np.dot(problem.Jac(x + eps3 * b, lmbda), z) / eps3 + np.dot(problem.Jac(x, lmbda + eps1), z) / eps1 - np.dot(J, z) * (1.0 / eps3 + 1.0 / eps1)
    ...
    # Update values
    dlmbda = (np.dot(phi, c) * np.dot(phi, f) - np.dot(phi, e) * np.dot(phi, d) + np.dot(phi, d)) / (np.dot(phi, d) * np.dot(phi, g) - np.dot(phi, c) * np.dot(phi, h))
    ...
    J = problem.Jac(x, lmbda)
    Rx = problem.Res(x, lmbda)
    Ry = np.dot(J, y) + omega * z # here I assume that B is the identity matrix (neglect thermal pressurization)
    Rz = np.dot(J, z) - omega * y # here I assume that B is the identity matrix (neglect thermal pressurization)
    Romega = np.dot(phi, y) - 1.0
    Rlmbda = np.dot(phi, z)
